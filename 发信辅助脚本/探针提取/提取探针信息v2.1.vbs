...
Sub ChooseSS()
	Dim WS,fso,oMDExcel,oHZExcel,bool,MySelfPath,TableHZ,TableMD,MDRow,n
	Set WS = WScript.CreateObject("Wscript.Shell")
	Set fso = WScript.CreateObject("Scripting.Filesystemobject")
	Set oHZExcel = WScript.CreateObject("Excel.Application")
	Set oMDExcel = WScript.CreateObject("Excel.Application")
	MySelfPath=fso.GetFile(Wscript.ScriptFullName).ParentFolder & "\"
	
	If Not fso.FileExists("名单.xlsx") Then
		WS.Popup "没找到【名单.xlsx】文件" & vbCrLf & "脚本将退出!",30,"文件未找到",4096+16
		WScript.Quit
	ElseIf Not fso.FileExists("信息汇总.xlsx") Then 
		WS.Popup "没找到【信息汇总.xlsx】文件" & vbCrLf & "脚本将退出!",30,"文件未找到",4096+16
		WScript.Quit
	Else
		Set TableMD=oMDExcel.Workbooks.Open(MySelfPath &"名单.xlsx").Worksheets
		Bool=False 
		For i = 1 To oMDExcel.Worksheets.Count
			If oMDExcel.Worksheets.Item(i).Name="总名单" Then 
				oMDExcel.Worksheets("总名单").Activate
				Bool=True 
				Exit For
			End If
		Next
		If Not Bool Then 
			WS.Popup "【名单.xlsx】文件中没有【总名单】表" & vbCrLf & "脚本将退出!",30,"表未找到",4096+16
			oMDExcel.ActiveWorkBook.Save
			oMDExcel.WorkBooks.Close
			oMDExcel.Quit
			WScript.Quit
		End If 
	End If 
	Set TableHZ=oHZExcel.Workbooks.Open(MySelfPath &"信息汇总.xlsx").Worksheets
	n=1
	Do Until oHZExcel.Cells(n,5).value=""
		MDRow=0
		If FindString(oHZExcel.Cells(n,5).value,"((\w+([-+.]\w+)*)|(\x22\w+([-+.]\w)*\x22))@((\w+([-.]\w+)*\.\w+([-.]\w+)*)|(\x5B\d+\.\d+\.\d+\.\d+\x5D))")<>"" Then MDRow=ExcelFindRow(oMDExcel.Cells,oHZExcel.Cells(n,5).value)
		If MDRow <> 0 Then 
			WtCompName oMDExcel,MDRow,oHZExcel
		End If 
		n=n+1
	Loop 
	Set fso=Nothing 
...
Function WtCompName(oMDExcel,MDRow,oHZExcel)
		oHZExcel.Cells(MDRow,1).value=oMDExcel.Cells(MDRow,2)
		oHZExcel.Cells(MDRow,2).value=oMDExcel.Cells(MDRow,4)
		oHZExcel.Cells(MDRow,3).value=oMDExcel.Cells(MDRow,30)
		oHZExcel.Cells(MDRow,4).value=oMDExcel.Cells(MDRow,32)
End Function
...
Function ExcelFindRow(ExcelCells,Str)
	On Error Resume Next 
	ExcelFindRow=ExcelCells.Find(Str).Row
End Function
